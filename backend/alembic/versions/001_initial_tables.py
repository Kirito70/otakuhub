...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("user_id", sa.Uuid(), sa.ForeignKey("users.id", ondelete="RESTRICT"), nullable=False),
        sa.Column("parent_reply_id", sa.Uuid(), sa.ForeignKey("discussion_replies.id", ondelete="SET NULL"), nullable=True),
        sa.Column("body", sa.Text(), nullable=False),
        sa.Column("has_spoilers", sa.Boolean(), nullable=False, server_default="0"),
        sa.Column("created_at", sa.DateTime(), nullable=False, server_default=sa.func.now()),
        sa.Column("updated_at", sa.DateTime(), nullable=False, server_default=sa.func.now()),
    ...
    op.create_table(
        ...
        sa.Column("related_media_id", sa.Uuid(), sa.ForeignKey("media_entries.id", ondelete="SET NULL"), nullable=True),
        sa.Column("related_user_id", sa.Uuid(), sa.ForeignKey("users.id", ondelete="SET NULL"), nullable=True),
